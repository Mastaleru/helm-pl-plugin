function portForward(config, callback) {
    ...
    if (typeof config.fullnameOverride !== "undefined") {
        child = childProcess.spawn("minikube", ["kubectl", "port-forward",  `deployment/${config.fullnameOverride}`, `8545:8545`], {detached: true});
    } else if (typeof config.pod_name !== "undefined") {
        child = childProcess.spawn("minikube", ["kubectl", "port-forward",  `${config.pod_name}`, `8545:8545`], {detached: true});
    } else {
